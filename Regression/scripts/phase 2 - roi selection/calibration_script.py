...
Per-dataset calibration of raw focus/tissue priors and sparsity thresholds.
...
def compute_hybrid_sparse_score(R10: float, Neff_norm: float, FF: float, EO: float,
                                calib_meds: Dict[str, float], calib_iqrs: Dict[str, float]) -> float:
    """
...
def composite_focus_measure(gray: np.ndarray) -> float:
    ...
    # GP expression:
    # if_gt(Threshold Absolute Gradient,
    #       Image Entropy / (Gradient Squared Energy + Fourier Transform Energy Index),
...
def threshold_absolute_gradient(gray: np.ndarray, threshold: float) -> float:
    ...
    # Match roi_selection_v2: return fraction of pixels above threshold (0..1).
...
# Main calibration
# ----------------------------
def calibrate(manifest: str, dataset_name: str, path_col: str, max_images: int, seed: int, out_csv: str):
    ...
    for row in rows:
        ...
        f_arr = np.zeros(len(tiles), dtype=np.float32)
        ...
        for i, t in enumerate(tiles):
            gray = to_gray(t)
            f_arr[i] = composite_focus_measure(gray)
    ...
    # Diagnostics: catch degenerate distributions early
    if np.allclose(np.std(F_cat), 0.0):
        print("[WARN] F_raw appears degenerate (std ~ 0). Check composite_focus_measure() and TAG normalization.")
    ...
    # Second pass: sparsity metrics with calibrated S
    R10_list = []
    Neff_list = []
    sumS_list = []
    maxS_list = []
    ...
    # robust med/iqr for hybrid normalization
    ...
    # hybrid sparse score thresholds (25/75 like R10)
    hybrid_scores = [
    ...
    print(f"  Hybrid: FF med/iqr={meds['FF_median']:.4f}/{iqrs['FF_iqr']:.4f}, EO med/iqr={meds['EO_median']:.4f}/{iqrs['EO_iqr']:.4f}")
...
def parse_args():
    ap = argparse.ArgumentParser(description="Dataset calibration for RIN priors and sparsity thresholds")
...
def main():
    ...
    calibrate(args.manifest, args.dataset_name, args.path_col, args.max_images, args.seed, out_csv)
