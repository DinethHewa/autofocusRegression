#!/usr/bin/env python3
"""
Builds a BMA-only manifest by:
- Discovering 14-slice stacks in BMA_ROOT.
- Dropping the first and last slice → operate on 12 slices.
...
- Writing:
    * Manifest CSV with columns [image_path, defocus_um] to OUT_DIR/manifest_bma.csv
    * Per-stack focus curves CSVs to REGRESSION_CURVES_DIR/BMA/<stack_id>_curves.csv
    * Full table (stack_id, image_name, image_path, defocus_um) to REGRESSION_TABLES_DIR/BMA/bma_table.csv

Usage:
    python manifest_creation_bma.py [--use-gpu]
"""
...
# Configuration
# ------------------------------------
BMA_ROOT = "/home/dineth/focus_measure/datasets/New folder/bma pbf tfa/bma_imgs"
...
CURVE_SUBDIR = os.path.join(REGRESSION_CURVES_DIR, "BMA")
TABLE_SUBDIR = os.path.join(REGRESSION_TABLES_DIR, "BMA")
MANIFEST_PATH = os.path.join(OUT_DIR, "manifest_bma.csv")
TABLE_PATH = os.path.join(TABLE_SUBDIR, "bma_table.csv")
...
def imread_gray(path: str) -> np.ndarray:
    ...
    if img is not None:
        ...
        return img
    # Fallback to PIL (tolerates CRC/truncated images)
    try:
...
def discover_bma_stacks(root: str) -> List[StackInfo]:
    ...
    for sd in sorted([d for d in os.listdir(root) if os.path.isdir(os.path.join(root, d))]):
        ...
        if TRIM_FIRST_LAST and len(imgs_sorted) >= 2:
            imgs_sorted = imgs_sorted[1:-1]
        if len(imgs_sorted) != EXPECTED_SLICES - 2:
            continue
...
# Core processing
# ------------------------------------
def process_stack(stack: StackInfo, fm: FocusMeasures) -> Tuple[int, Dict[str, np.ndarray], Dict[str, int], List[str]]:
...
def build_manifest(use_gpu: bool = False, flush_every: int = 200) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ensure_dirs()
    stacks = discover_bma_stacks(BMA_ROOT)
    if not stacks:
        raise RuntimeError(f"No valid stacks (exactly {EXPECTED_SLICES} images) found in {BMA_ROOT}")
    ...
    for stack in tqdm(stacks, desc="Processing stacks"):
        ...
        for i, img_path in enumerate(valid_paths):
            defocus_um = (focus_idx - i) * STEP_BMA
...
def main():
    parser = argparse.ArgumentParser(description="Create BMA manifest with focus-curve voting.")
