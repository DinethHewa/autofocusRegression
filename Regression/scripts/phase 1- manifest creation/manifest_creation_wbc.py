#!/usr/bin/env python3
"""
Builds a WBC-only manifest by:
- Discovering 10-slice stacks in WBC_dataset1.
...
- Assigning defocus distances (µm) using 0.4 µm step relative to the voted focus slice.
- Writing:
    * Manifest CSV with columns [image_path, defocus_um] to OUT_DIR/manifest_wbc.csv
    * Per-stack focus curves CSVs to REGRESSION_CURVES_DIR/WBC/<stack_id>_curves.csv
    * Full table (same columns as manifest + stack_id) to REGRESSION_TABLES_DIR/WBC/wbc_table.csv

Usage:
    python manifest_creation_wbc.py [--use-gpu]
"""
...
# Configuration
# ------------------------------------
WBC_ROOT = "/home/dineth/focus_measure/datasets/WBC_dataset1"
...
CURVE_SUBDIR = os.path.join(REGRESSION_CURVES_DIR, "WBC")
TABLE_SUBDIR = os.path.join(REGRESSION_TABLES_DIR, "WBC")
MANIFEST_PATH = os.path.join(OUT_DIR, "manifest_wbc.csv")
TABLE_PATH = os.path.join(TABLE_SUBDIR, "wbc_table.csv")
...
def discover_wbc_stacks(root: str) -> List[StackInfo]:
    ...
    for sd in sorted([d for d in os.listdir(root) if os.path.isdir(os.path.join(root, d))]):
        ...
        if len(imgs) != EXPECTED_SLICES:
            continue
        stacks.append(StackInfo(stack_id=sd, slice_paths=sort_by_trailing_number(imgs)))
    return stacks
...
class FocusMeasures:
    ...
    def glcm_contrast(self, image):
        # Always operate in NumPy uint8 for skimage GLCM
        if not HAVE_SKI:
...
def process_stack(stack: StackInfo, fm: FocusMeasures) -> Tuple[int, Dict[str, np.ndarray], Dict[str, int]]:
    ...
    if len(candidate_indices) == 1:
        ...
        # tie-breaker: pick slice with highest summed oriented values; then closest to center; then lowest idx
    ...
    return focus_idx, oriented_curves, best_indices


def build_manifest(use_gpu: bool = False, flush_every: int = 200) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ensure_dirs()
    stacks = discover_wbc_stacks(WBC_ROOT)
    if not stacks:
        raise RuntimeError(f"No valid stacks (exactly {EXPECTED_SLICES} images) found in {WBC_ROOT}")

    fm = FocusMeasures(use_gpu=use_gpu)
    # Resume support: load existing outputs if present
    ...
    for stack in tqdm(stacks, desc="Processing stacks"):
        ...
        focus_idx, oriented_curves, best_indices = process_stack(stack, fm)

        # Save curves CSV
        curve_path = os.path.join(CURVE_SUBDIR, f"{stack.stack_id}_curves.csv")
        curve_df = pd.DataFrame({"slice_index": list(range(len(stack.slice_paths)))})
        curve_df["image_name"] = [os.path.basename(p) for p in stack.slice_paths]
        ...
        xs = list(range(len(stack.slice_paths)))
        for name, curve in oriented_curves.items():
        ...
        for i, img_path in enumerate(stack.slice_paths):
            defocus_um = (focus_idx - i) * STEP_WBC
        ...
        if flush_every > 0 and processed_since_flush >= flush_every:
            # periodic flush to disk for resilience
...
def main():
    parser = argparse.ArgumentParser(description="Create WBC manifest with focus-curve voting.")
    parser.add_argument("--use-gpu", action="store_true", help="Use CuPy if available for select ops.")
    parser.add_argument("--flush-every", type=int, default=200, help="Flush manifest/table every N stacks (default 200).")
