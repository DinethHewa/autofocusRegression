#!/usr/bin/env python3
"""
Builds a TBF-only manifest by:
- Discovering stacks in TBF_ROOT with exactly 14 slices (keeps all slices).
...
- Writing:
    * Manifest CSV with columns [image_path, defocus_um] to OUT_DIR/manifest_tbf.csv
    * Per-stack focus curves CSVs to REGRESSION_CURVES_DIR/TBF/<stack_id>_curves.csv
    * Per-stack focus curve PNGs to the same folder
    * Full table (stack_id, image_name, image_path, defocus_um) to REGRESSION_TABLES_DIR/TBF/tbf_table.csv

Usage:
    python manifest_creation_tbf.py [--use-gpu] [--flush-every N]
"""
...
# Configuration
# ------------------------------------
TBF_ROOT = "/home/dineth/focus_measure/datasets/New folder/bma pbf tfa/tbf_imgs"
...
CURVE_SUBDIR = os.path.join(REGRESSION_CURVES_DIR, "TBF")
TABLE_SUBDIR = os.path.join(REGRESSION_TABLES_DIR, "TBF")
MANIFEST_PATH = os.path.join(OUT_DIR, "manifest_tbf.csv")
TABLE_PATH = os.path.join(TABLE_SUBDIR, "tbf_table.csv")
...
def imread_gray(path: str) -> np.ndarray:
    ...
    if img is not None:
        ...
        return img
    # Fallback to PIL to tolerate CRC/truncated images
    try:
...
def discover_tbf_stacks(root: str) -> List[StackInfo]:
...
def build_manifest(use_gpu: bool = False, flush_every: int = 200) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ensure_dirs()
    stacks = discover_tbf_stacks(TBF_ROOT)
    if not stacks:
        raise RuntimeError(f"No valid stacks (exactly {EXPECTED_SLICES} images) found in {TBF_ROOT}")
    ...
    for stack in tqdm(stacks, desc="Processing stacks"):
        ...
        for i, img_path in enumerate(valid_paths):
            defocus_um = (focus_idx - i) * STEP_TBF
...
def main():
    parser = argparse.ArgumentParser(description="Create TBF manifest with focus-curve voting.")
    ...
    print(f"Curve CSV/PNGs in {CURVE_SUBDIR}")
