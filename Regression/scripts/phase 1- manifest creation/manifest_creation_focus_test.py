#!/usr/bin/env python3
"""
Manifest creator for the focusTest dataset (Windows paths).
- Scans multiple root folders (recursively) for image files.
- Extracts defocus distance from filenames like: defocus150.jpg -> 0.15, defocus-350.jpg -> -0.35, defocus4650.jpg -> 4.65.
- Writes manifest_focus_test.csv with columns [image_path, defocus_um] to OUT_DIR.
- Resume-aware and flushes periodically to avoid losing progress.

Usage:
    python manifest_creation_focus_test.py [--flush-every N] [--roots ROOT1 ROOT2 ...]
"""
...
# --------------------------------------------------
DEFAULT_ROOTS = [
    r"D:\Academic\Courses\Higher Education\Design and Development of Autofocus and Auto selection Microscopy\dataset\5936881 (1)\Data_channel\incoherent_RGBchannels\testRawData_incoherent_diffProtocol",
    r"D:\Academic\Courses\Higher Education\Design and Development of Autofocus and Auto selection Microscopy\dataset\5936881 (1)\Data_channel\incoherent_RGBchannels\testRawData_incoherent_sameProtocol",
    r"D:\Academic\Courses\Higher Education\Design and Development of Autofocus and Auto selection Microscopy\dataset\5936881 (1)\Data_domain_part1\incoherent_2domains\testRawData_incoherent_diffProtocol",
    r"D:\Academic\Courses\Higher Education\Design and Development of Autofocus and Auto selection Microscopy\dataset\5936881 (1)\Data_domain_part1\incoherent_2domains\testRawData_incoherent_sameProtocol",
]

OUT_DIR = "/home/dineth/focus_measure/journal/Regression/data"
MANIFEST_PATH = os.path.join(OUT_DIR, "manifest_focus_test.csv")
...
def normalize_root_path(path: str) -> str:
    ...
    if os.path.isdir(path):
        return path
    # map Windows drive to /mnt/<drive>
...
def parse_defocus_from_name(filename: str):
    """
    Extract defocus value from names like defocus150, defocus-350, defocus4650, defocus_150.
    ...
    if not m:
        return None
    val_str = m.group(1)
    # Normalize various minus/dash characters to ASCII minus
    for ch in ["−", "–", "—", "\u2212", "\u2013", "\u2014"]:
...
def main():
    parser = argparse.ArgumentParser(description="Create focusTest manifest by parsing defocus from filenames.")
