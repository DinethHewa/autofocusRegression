...
class CenterCropBaseline(BaselineMethod):
    baseline_name = 'center_crop_inference_only'
    ...
    def describe(self) -> dict:
        return {
            ...
            'note': 'Uses the fixed proposed downstream models but replaces content-aware ROI selection with the deterministic center-most ROI.',
        }
...
def main() -> None:
    ap = argparse.ArgumentParser(description='Run center-crop inference-only baseline')
    ap.add_argument('--track', required=True, choices=['smears', 'biopsy'])
    ap.add_argument('--seed', type=int, default=42)
    ap.add_argument('--resume', action='store_true')
    args = ap.parse_args()
    baseline = CenterCropBaseline(track=args.track, seed=args.seed, resume=args.resume)
    bundle = baseline.predict_fov()
    print(f'[DONE] Center-crop baseline FOV predictions: {baseline.paths["inference"] / "fov_predictions.csv"}')
    print(f'[DONE] Center-crop baseline rows: {len(bundle.fov_df)}')
